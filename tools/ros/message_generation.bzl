...
def _gencpp_impl(ctx):
    ...
    for dep in ctx.attr.deps:
        dep_srcs = dep_srcs + dep[GenMsgProvider].srcs
        dep_flags.append('-I')
        dep_flags.append('%s:%s' % (dep[GenMsgProvider].package_name ,dep[GenMsgProvider].srcs[0].dirname))
    ...
    return [GenMsgProvider(package_name=ctx.attr.ros_package_name, srcs=ctx.files.srcs)]
